...
function Report() {
  ...
  const loadAttempts = async () => {
    ...
    try {
      ...
      console.log("DATA:",data)
      let mapped = (data.attempts || []).map(a => {
          const resultsArr = Array.isArray(a.results_data) ? a.results_data : (a.results_data ? Object.values(a.results_data) : []);
          return {
            id: a.id || a.candidate_id || '—',
            candidateId: a.candidate_id || a.details?.candidate_id || null,
            cid: a.cid ?? a.details?.cid ?? null,
            name: 'Candidate',
            email: '',
            company: a.details?.company || a.company || '-',
            jobTitle: a.details?.role_title || a.role_title || a.title || '-',
            totalQuestion: resultsArr.length || '-',
            marks: resultsArr.length ? (() => {
              try {
                let obtained = 0;
                let possible = 0;
                ...
                for (const q of resultsArr) {
                  ...
                  // Score is typically returned from backend evaluation as a float 0.0-1.0
                  // or pre-scaled by positive_marking in submit_section
                  const rawScore = q.score ?? q.raw_score ?? 0;
                  const posMark = Number(q.positive_marking ?? q.max_score ?? 1);
                  
                  // Backend logic in submit_section scales score by positive_marking:
                  // final_score = round(norm_score * pos_mark, 3)
                  // So we can sum 'score' directly.
                  obtained += Number(rawScore) || 0;
                  possible += posMark;
                }

                return `${obtained.toFixed(2).replace(/\.00$/, '')}/${possible}`;
              } catch (e) {
                return '-';
              }
            })() : '-',
            skills: Array.isArray(a.details?.skills) ? a.details.skills : (a.details?.skills || a.skills || []),
            time: '-',
            correct: '-',
            incorrect: '-',
            raw: a,
